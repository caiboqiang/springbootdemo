...
function registerResponse(message) {
    if (message.response == 'accepted') {
        ...
        document.getElementById('name').focus();
        alert('1Error registering user. See console for further information.(错误的注册用户:可能用户一存在)');
    }
}

function callResponse(message) {
    if (message.response != 'accepted') {
        console.info('2 Call not accepted by peer. Closing call（呼叫不被同行接受。关闭电话）');
        stop();
        setCallState(NO_CALL);
        if (message.message) {
            alert("3"+message.message+"---------------------------");
        }
    } else {
...
function register() {
    ...
    if (name == '') {
        window.alert('5：You must insert your user name（必须插入用户名）');
        document.getElementById('name').focus();
        return;
    }
...
function call() {
    if (document.getElementById('peer').value == '') {
        document.getElementById('peer').focus();
        window.alert('6 You must specify the peer name（您必须指定对等名称：呼叫用户名）');
        return;
    }
    setCallState(DISABLED);
    showSpinner(videoInput, videoOutput);
...
function play() {
    ...
    if (peer == '') {
        window
            .alert("7：You must insert the name of the user recording to be played (field 'Peer')：（必须插入要播放的用户记录的名称(字段'Peer')）");
        document.getElementById('peer').focus();
        return;
    }
